...
const AdminDashboard = () => {
    ...
    const handleDepositAction = async (transactionId, action) => {
        ...
        try {
            ...
            await axios.put(`/api/admin/deposits/${transactionId}/${action}`, {
                declineReason: action === 'decline' ? declineReason : undefined
            }, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleAddBalance = async (e) => {
        ...
        setLoading(true);
        try {
            await axios.post('/api/admin/add-balance', { userId: selectedUser, amount }, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleUpdateUser = async (e) => {
        ...
        try {
            ...
            await axios.put(`/api/admin/users/${editUserData.id}`, updateData, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleAddBalanceFromEdit = async (e) => {
        ...
        setLoading(true);
        try {
            await axios.post('/api/admin/add-balance', { userId: editUserData.id, amount }, {
                headers: { Authorization: `Bearer ${token}` }
            });
            setAmount('');
            fetchData();
            // Update the editUserData balance
            const updatedUser = await axios.get('/api/admin/users', {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleUpdateTransaction = async (e) => {
        ...
        setLoading(true);
        try {
            const response = await axios.put(`/api/admin/transactions/${editingTransaction.id}`, editingTransaction, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleDeleteTransaction = async () => {
        setLoading(true);
        try {
            const response = await axios.delete(`/api/admin/transactions/${deletingTransaction._id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
